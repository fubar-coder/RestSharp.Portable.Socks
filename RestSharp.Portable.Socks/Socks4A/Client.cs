...
namespace RestSharp.Portable.Socks.Socks4A
{
    public class Client : ITcpClient
    {
        ...
        private string GetUserId()
        {
            if (Credentials == null)
                return string.Empty;
            var socksCredentials = Credentials.GetCredential(new UriBuilder("socks4a", SocksAddress.Host, SocksAddress.Port).Uri, "Plain");
            if (socksCredentials == null)
                socksCredentials = Credentials.GetCredential(new UriBuilder("socks4", SocksAddress.Host, SocksAddress.Port).Uri, "Plain");
            if (socksCredentials == null)
                return string.Empty;
